...
const getBaseUrl = (envValue: string | undefined, defaultBase: string) => {
    ...
    // Fix swapped environment variable names if necessary
    if (defaultBase.includes('all_payments') && clean.endsWith('/payments')) {
        return defaultBase;
    }
    if (defaultBase.endsWith('/payments') && clean.includes('all_payments')) {
        return defaultBase;
    }
